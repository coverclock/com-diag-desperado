...
/******************************************************************************
    ...
    $Id: unittestEscape.cpp,v 1.3 2006/01/09 00:24:49 jsloan Exp $
 ...
 *  @file
 *
 *  Implements the Escape unit test.
 *
 *  @see    Escape
 *
...
extern "C" int unittestEscape() {
    ...
    size_t ne = escape.expand(raw, sizeof(raw), expanded, sizeof(expanded));
    ...
    size_t nc = escape.collapse(expanded, collapsed, sizeof(collapsed));
    ...
    for (size_t limit = 0; sizeof(expanded) > limit; ++limit) {
        ...
        expanded[limit] = '!';
        expanded[0] = '?';
        ne = escape.expand(raw, sizeof(raw), expanded, limit);
        ...
        if (0 == limit) {
            ...
            nc = escape.collapse(expanded, collapsed, nc);
    ...
    for (size_t limit = 0; sizeof(collapsed) > limit; ++limit) {
        ...
        collapsed[limit] = '!';
        collapsed[0] = '?';
        nc = escape.collapse(expanded, collapsed, limit);
    ...
    const char invalid[] = "\\e\\x0g\\xh0\\xgh\\09\\009\\0009\\00009";
    const char result[] = {
        '\\', 'e',
        '\0', 'g',
        '\\', 'x', 'h', '0',
    ...
    nc = escape.collapse(invalid, collapsed, sizeof(collapsed));
    if (sizeof(result) != nc) {
        errorf("%s[%d]: (%u?%u)!\n",
